...
private[example] trait AnfTransform {
  self: ActMMacro =>
    ...
    typingTransform(block)((tree, api) => {
      ...
      object linearize {
        ...
        def _transformToList(tree: Tree): List[Tree] = trace(tree) {
          ...
          expr match {
            case Select(Apply(Apply(fun, List(arg)), List(unapply)), op) if isNext(fun, op) =>
              val valDef = defineVal(name.next, expr, tree.pos)
      ...
      object anf {
        ...
        def _transformToList(tree: Tree): List[Tree] = trace(tree) {
          val containsNext = tree exists isMonadOp
          if (!containsNext) {
            tree match {
